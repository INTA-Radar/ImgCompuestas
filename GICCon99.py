...
#			Este scrit NO filtra los -99 en los calculos, por el resto es igual a GIC.py. 
#			Todo: juntarlo con GIC y que el filtro de los -99 sea un parámetro.
import sys, os
import argparse
...
def gen_unaelevacion(imgs, args):
	...
	if args.prom:
		...
		B=numpy.mean(A, axis=0,dtype=np.float64)
		archivo= args.path_img+args.fecha+'AVG'+args.variable+elevacion+'.tif'
		save_imgcomp(imagen, archivo, B, numpy.nan)
	if args.tot:
	...
	if args.maxi:
		...
		save_imgcomp(imagen, archivo, datosmaximos, numpy.nan)
	if args.mini:
		archivo= args.path_img+args.fecha+'MIN'+args.variable+elevacion+'.tif'
		save_imgcomp(imagen, archivo, datosminimos, numpy.nan)
	return 
...
if args.mto:
	...
	if args.ele == 0:
		...
		print 'max',len(imgsmax)
		print 'min',len(imgsmin)
		print 'prom',len(imgsprom)
		print 'tot',len(imgstot)
		raw_input()
		if args.mini:
			try:
		...
		print 'cantidad imagenes',len(imgs)
		raw_input()
		print imgs
		try:
